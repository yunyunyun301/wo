...
def func(x):
    return x**2
li = [1,2,3]
print(map(func,li)) #<map object at 0x0000021BC80>参数1是函数名，参数2是可迭代对象
...
4. write()
    * 将指定内容写入文件，写入的内容必须是字符串类型，如果不是字符串类型，需要先进行转换
...
* . :指定小数点后位数
```py
print(f'姓名:{name:<10},年龄:{age:>5}')  # 左对齐和右对齐
print(f'姓名:{name:^10},年龄:{age:0>5}')  # 居中对齐和用0填充空白
print(f'姓名:{name},年龄:{age:.2f}')  # 指定小数点后位数
```
